...
`;
    ...
    try {
      ...
      if (!parsed.careerPath || !Array.isArray(parsed.careerPath)) {
  ...
  return (
    ...
    >
      ...
      {error && (
        <p
          className="text-center text-red-600 font-semibold mb-6 text-base"
          role="alert"
        >
          {error}
        </p>
        ...
        >
          ...
          <div className="relative before:absolute before:w-1 before:bg-blue-600 before:rounded-full before:h-full before:left-10 before:top-0">
            {timeline.map(({ step, title, description, duration }) => (
              ...
              >
                <div
                  className="absolute left-0 top-8 flex items-center justify-center w-10 h-10 rounded-full border-4 border-blue-600 bg-blue-100 text-blue-600 font-bold text-lg select-none"
                >
                  {step}
                </div>
